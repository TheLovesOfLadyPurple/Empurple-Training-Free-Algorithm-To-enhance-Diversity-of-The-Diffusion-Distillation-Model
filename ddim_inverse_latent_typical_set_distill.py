...
def main():
    ...
    parser.add_argument("--mode", type=str, choices=["intermediate", "final_x0"], default="final_x0", 
                        help="'intermediate': use stored z_t directly; "
                             "'final_x0': load predicted x0 and add noise before solving")
    parser.add_argument("--outdir", type=str, default="./gen_img_val_inverse_typical_set")
    parser.add_argument("--prompt_dir", type=str,
                        default="./gen_img_val_v15_coco2014_unipc_low_new_c/prompt-12-5.5",
                        help="Unused compatibility option; prompt matching is disabled.")
    parser.add_argument("--latent_dir", type=str, default='./gen_img_val_v15_coco2014_unipc_low_new_c_val/final_x0-12-5.5',
    ...
    parser.add_argument("--compare_cosine_similarity", type=bool, default=False,
                        help="Unused compatibility flag; latents are loaded directly by index.")

    opt = parser.parse_args()

    # Auto-resolve latent directory
    if opt.latent_dir is None:
        base = "./gen_img_val_v15_coco2014_unipc_low_new_c"
    ...
    # ------------------------------------------------------------------
    # 3. Load COCO captions as query prompts
    # ------------------------------------------------------------------
    ...
    folder_name = f"samples-{opt.mode}-ts{opt.noise_timestep:05}-step-{opt.ddim_steps}-random"
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                for prompts in tqdm(grouped, desc="data"):
                    ...
                    for sample_idx in range(len(prompts)):
                        latent_name = f"{base_count + sample_idx:05}_{opt.latent_timestep}.pth"
                        latent_file = os.path.join(opt.latent_dir, latent_name)
                        if not os.path.exists(latent_file):
                            raise FileNotFoundError(f"Expected latent file not found: {latent_file}")

                        latent = torch.load(latent_file, map_location='cpu', weights_only=True)
                    ...
                    if True:
                        ...
                        for img in samples_ddim:
                            img = img.permute(1, 2, 0)
                            sample_name = f"{base_count:05}_{opt.noise_timestep:05}.pth"
                            torch.save(img, os.path.join(sample_path, sample_name))
                            base_count += 1
                        base_count = cache_tmp_count
                        for img in c:
                            prompt_name = f"{base_count:05}_{opt.noise_timestep:05}.pth"
                            torch.save(img, os.path.join(prompt_save_path, prompt_name))
                            base_count += 1
