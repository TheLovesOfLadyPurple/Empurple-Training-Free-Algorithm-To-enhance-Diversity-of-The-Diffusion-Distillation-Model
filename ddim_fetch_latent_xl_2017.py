...
class DDIMSolverXL:
    def __init__(self
        ...
        self.ddim_timesteps = (np.arange(0, ddim_timesteps + 1) * step_ratio).round().astype(np.int64) - 1
        if is_abalation:
            self.ddim_timesteps = np.append(self.ddim_timesteps, 999)
        self.ddim_timesteps = np.flip(self.ddim_timesteps).copy()
        self.ddim_timesteps[-1] = self.ddim_timesteps[-1] + 1
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="./gen_img_val_fetch_latent_val_2017_xl_hyper")
    ...
    parser.add_argument("--use_decode_net_type", type=str, default="SDXL-lightning",
                        choices=["Flash", "LCM","SDXL-lightning","DMD2","Hyper-SD"],)
    parser.add_argument("--from-instances-file", type=str, default="./instances_val2017.json")
    parser.add_argument("--from-caption-file", type=str, default="./captions_val2017.json")
    ...
    if opt.use_decode_net_type == "Flash":
        ...
        inference_step = 4
    elif opt.use_decode_net_type == "Hyper-SD":
        repo_id = "ByteDance/Hyper-SD"
        base = "stabilityai/stable-diffusion-xl-base-1.0"
        ckpt = "Hyper-SDXL-4steps-lora.safetensors" # Use the correct ckpt for your step setting!
        pipe = StableDiffusionXLPipeline.from_pretrained(base, vae=vae, torch_dtype=DTYPE).to("cuda")
        pipe.load_lora_weights(hf_hub_download(repo_id, ckpt))
        pipe.fuse_lora()
        inference_step = 4
    elif opt.use_decode_net_type == "LCM":
        repo_id = "latent-consistency/lcm-sdxl"
    if opt.use_decode_net_type != "SDXL-lightning" and opt.use_decode_net_type != "Hyper-SD":
