...
def pth_writer_worker(save_queue):
    while True:
        ...
        save_type, payload, file_path = item
        if save_type == "png_image":
            Image.fromarray(payload).save(file_path)

def queue_tensor_save(save_queue, tensor: torch.Tensor, file_path: str):
    save_queue.put((tensor.detach().cpu(), file_path))

def queue_image_save(save_queue, image_tensor: torch.Tensor, file_path: str):
    image = 255. * rearrange(image_tensor.detach().cpu().numpy(), 'c h w -> h w c')
    image = np.clip(image, 0, 255).astype(np.uint8)
    save_queue.put(("png_image", image, file_path))
...
def main():
    ...
    parser.add_argument(
        ...
        default="./gen_img_train_xl_coco2014_unipc"
    )
    parser.add_argument(
        "--skip_save",
        action='store_true',
    ...
    save_queue = mp.Queue(maxsize=128)
    writer_process = mp.Process(target=pth_writer_worker, args=(save_queue,))
            ...
            with precision_scope("cuda"):
                ...
                for n in trange(opt.n_iter, desc="Sampling", disable =not accelerator.is_main_process):
                    ...
                    # if opt.method == "dpm_solver_v3":
                        ...
                        prompt_embeds, _ = prepare_sdxl_pipeline_step_parameter(pipe, prompts, opt.scale != 1.0, device, batch_size * [""] if opt.scale != 1.0 else None)
                    # We only want the positive prompt embeds
                        if opt.scale != 1.0:
                            prompt_embeds_save = prompt_embeds.chunk(2)[1]
                        else:
                            prompt_embeds_save = prompt_embeds
                        ...
                        imgs = grather_feature_dict['intermediate_x']
                        cache_tmp_count = direct_distill_intermediate_count
                        for idx, imgs in enumerate(grather_feature_dict['intermediate_x']):
                            if idx < 1 or idx > 3:
                                continue
                            time_tag = int(grather_feature_dict['tmp_t'][idx][0])
                            for img in imgs:
                                img = img.permute(1,2,0)
                                torch.save(img,os.path.join(intermediate_path, f"{direct_distill_intermediate_count:05}_{time_tag:05}.pth"))
                                direct_distill_intermediate_count += 1
                            direct_distill_intermediate_count = cache_tmp_count
                            for img in prompt_embeds_save:
                                torch.save(img,os.path.join(prompt_path, f"{direct_distill_intermediate_count:05}_{time_tag:05}.pth"))
                                direct_distill_intermediate_count += 1
                            direct_distill_intermediate_count = cache_tmp_count
                            for img in samples:
                                img = img.permute(1,2,0)
                                torch.save(img,os.path.join(final_x0_path, f"{direct_distill_intermediate_count:05}_{time_tag:05}.pth"))
                                direct_distill_intermediate_count += 1
                            cache_tmp_count = direct_distill_intermediate_count
