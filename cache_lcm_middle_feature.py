...
"""
Although copy a new file is silly, 
but to prevent any possible change on the random effect, and to ensure the reproducibility, 
we copy it
"""
...
class LCMSDESolver:
    ...
    def solve(self, sample, prompt_embeds, scale=7.5):
        ...
        for idx, timestep in enumerate(self.ddim_timesteps):
            ...
            intermediate_state["noisy_z"].append(tmp_hidden_z)
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="./gen_img_val_fetch_latent_val_cache_noisy")
    ...
    parser.add_argument("--use_decode_net_type", type=str, default="LCM",
                        choices=["Flash", "LCM"])
    ...
    folder_name = f"samples-step-{opt.ddim_steps}-{opt.scale}-{opt.use_decode_net_type}"
    sample_path = os.path.join(opt.outdir, folder_name)
    noisy_feature_path = os.path.join(opt.outdir, folder_name.replace("samples-", "noisy_z-", 1))
    os.makedirs(sample_path, exist_ok=True)
    os.makedirs(noisy_feature_path, exist_ok=True)
        ...
        with precision_scope("cuda"):
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                for prompts in tqdm(grouped, desc="data"):
                    ...
                    for step_idx, noisy_batch in enumerate(guide_distill["noisy_z"]):
                        time_tag = int(solver.ddim_timesteps[step_idx + 1].item())
                        for sample_idx, feature in enumerate(noisy_batch):
                            feature_name = f"{feature_base_count + sample_idx:05}_{time_tag:05}.pth"
                            torch.save(
                                feature.detach().permute(1, 2, 0).cpu(),
                                os.path.join(noisy_feature_path, feature_name)
                            )
                    if not opt.force_org:
    ...
    print(f"Cached noisy features: {noisy_feature_path}")
